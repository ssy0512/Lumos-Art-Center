...
public class MyExcelView extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest req,
		...
		resp.setContentType("application/msexcel"); //application octet stream 파일명을 보낼것, 
	...
	// sheet 생성
	/**
	 * @param workbook 엑셀의 파일 하나를  book이라고 함. 
	 * @param sheetIx : 쉬트 인덱스(0부터 시작)
	 * @param sheetName
	 * @return
	 */
	private Sheet createSheet(Workbook workbook, int sheetIx, String sheetName) { //POI method referring 
	...
	private void createColumnValue(Sheet sheet, List<Object[]> columnValues) {
		...
		for(int idx=0; idx<columnValues.size(); idx++) { //1행에는제목 나머지는 내용 . index는 0부터 시작함.
