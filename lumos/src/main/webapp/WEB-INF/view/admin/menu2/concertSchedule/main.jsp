...
$(function() {
	calendar=$('#calendar').fullCalendar({
		...
		resources: {
			url: '<%=cp%>/admin/menu2/concertSchedule/resources',
			type:'post',
			error: function() {
				console.log("error...");
			}
		...
		events: function(start, end, timezone, callback){
			...
			var url= '<%=cp%>/admin/menu2/concertSchedule/events';
...
function insertForm(start, end, resource) {
	...
	if(resource) {
		...
		var url="<%=cp%>/admin/menu2/concertSchedule/readResourceList";
	...
	$('#resourceModal').load("<%=cp%>/admin/menu2/concertSchedule/inputForm", function() {
		var startDay="";
		var endDay="";
...
function changeGroup(groupNum, resourceNum) {
	...
	var url="<%=cp%>/admin/menu2/concertSchedule/listHall";
...
function insertOk() {
	...
	var url="<%=cp%>/admin/menu2/concertSchedule/schedulerInsert";
...
function validCheck() {
	...
	if(! title) {
	 ...
	 if(! /[12][0-9]{3}-[0-9]{2}-[0-9]{2}/.test(startDay)){
		alert("날짜를 정확히 입력해 주세요 [yyyy-mm-dd] ");
...
function articleForm(calEvent) {
	...
	$('#resourceModal').load("<%=cp%>/admin/menu2/concertSchedule/articleForm", function() {
...
function updateOk() {
	...
	var query=$("form[name=schedulerForm]").serialize();
	var url="<%=cp%>/admin/menu2/concertSchedule/schedulerUpdate";
...
function deleteOk(num) {
	if(confirm("삭제 하시겠습니까 ?")) {
		$.post("<%=cp%>/admin/menu2/concertSchedule/schedulerDelete", {num:num}, function(data){
			var state=data.state;
			if(state=="false") {
...
function updateDrag(calEvent) {
	...
	var url="<%=cp%>/adminmenu2/concertSchedule/schedulerUpdate";
...
<div class="body-container" style="width: 1400px; margin:0 auto; align:center;">
    
    <div class="body-title" style="height:90px;">
        <h2><span style="font-family: Webdings">2</span>공연 일정 관리 </h2>
    </div>
    
    <div>
        <div id='schLoading'>loading...</div>
        <div id='calendar' style="height: 35px; line-height: 35px;"></div>
    </div>
