...
function setScore() {
 ...
 function sendOk() {
     ...
     var mode="${mode}";
   	 ...
   	 f.action="<%=cp%>/exhibitReview/${mode}";
					...
					<tbody>
						...
						<tr>
							<td colspan="3" style="text-align: center; height: 80px;">
								...
								<button type="button" class="eBtn" onclick="javascript:location.href='<%=cp%>/exhibit/article?num=${dto.exhibitNum}';">${mode=='update'?'수정취소':'등록취소'}</button>
								
								 <c:if test="${mode=='update'}">
								         <input type="hidden" name="num" value="${rdto.exreviewNum}">
								</c:if>
